...
{
    if (!yoloDataset.useDatasetStructure) return;
...
void VulkanApplication::saveYOLOAnnotation(const std::string& filename)
{
    ...
    // Обновляем classes.txt в корне датасета
    std::string classesPath;
    if (yoloDataset.useDatasetStructure) {
        classesPath = yoloDataset.datasetPath + "/classes.txt";
    } else {
        classesPath = "classes.txt";
    }

    // Сначала читаем существующие классы
    std::vector<std::string> existingClasses;
    std::ifstream readClasses(classesPath);
    std::string line;

    while (std::getline(readClasses, line)) {
        if (!line.empty()) {
            existingClasses.push_back(line);
        }
    }
    readClasses.close();

    // Проверяем, нужно ли добавить новый класс
    ...
    // Перезаписываем classes.txt с правильным форматом
    std::ofstream writeClasses(classesPath);
    if (writeClasses.is_open()) {
        for (const auto& cls : existingClasses) {
            writeClasses << cls << std::endl;
        }
        writeClasses.close();
    }
...
void VulkanApplication::updateSelectionRect(const glm::vec3& modelPos, const glm::mat4& viewProj) {
    ...
    // Строим матрицу модели ТОЧНО ТАК ЖЕ, как в updateUniformData
    glm::mat4 modelMatrix = glm::mat4(1.0f);

    // 1. Сначала применяем масштабирование (локальное преобразование)
    modelMatrix = glm::scale(modelMatrix, glm::vec3(scale));

    // 2. Перемещаем модель так, чтобы её центр оказался в начале координат (локальное преобразование)
    modelMatrix = glm::translate(modelMatrix, translate);

    // 3. Перемещаем модель в мировые координаты (мировое преобразование) - ТОЛЬКО ОДИН РАЗ
    modelMatrix = glm::translate(modelMatrix, modelPos);

    // 4. ПРИМЕНЯЕМ ПОВОРОТ ПОСЛЕ ПЕРЕМЕЩЕНИЯ В МИРОВЫЕ КООРДИНАТЫ
    ...
    // УБИРАЕМ ВТОРОЕ ПРИМЕНЕНИЕ modelPos - оно здесь не нужно!
    ...
    // Обрезаем рамку по границам viewport'а
...
void VulkanApplication::updateUniformData()
{
    ...
    // 3. Перемещаем модель в мировые координаты (мировое преобразование)
    shaderValuesScene.model = glm::translate(shaderValuesScene.model, modelPosition);

    // 4. ПРИМЕНЯЕМ ПОВОРОТ ПОСЛЕ ПЕРЕМЕЩЕНИЯ В МИРОВЫЕ КООРДИНАТЫ
    // Это обеспечит вращение вокруг локального центра модели в мировом пространстве
...
void VulkanApplication::updateOverlay()
{
    ...
    if (ui->header("YOLO")) {
        ...
        if (saveMode == 1 || saveMode == 2) {
            ...
            ImGui::SameLine();
            if (ImGui::InputText("##datasetpath", datasetPathBuffer, sizeof(datasetPathBuffer))) {
...
bool VulkanApplication::isPositionVisible(const glm::vec3& pos)
{
    ...
    modelMatrix = glm::scale(modelMatrix, glm::vec3(scale));
    modelMatrix = glm::translate(modelMatrix, translate);
    modelMatrix = glm::translate(modelMatrix, pos);  // Перемещение в мировые координаты
    modelMatrix = glm::rotate(modelMatrix, glm::radians(modelRotation.x), glm::vec3(1.0f, 0.0f, 0.0f));
    modelMatrix = glm::rotate(modelMatrix, glm::radians(modelRotation.y), glm::vec3(0.0f, 1.0f, 0.0f));
    modelMatrix = glm::rotate(modelMatrix, glm::radians(modelRotation.z), glm::vec3(0.0f, 0.0f, 1.0f));
