...
public:
    ...
    // Структура для описания viewport'а UI
    struct UIViewport {
        uint32_t x;
        uint32_t y;
        uint32_t width;
    ...
    // Новая версия draw с поддержкой viewport'а
    void draw(VkCommandBuffer cmdBuffer, const UIViewport& viewport) {
        vkCmdBindPipeline(cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline);
        vkCmdBindDescriptorSets(cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &descriptorSet, 0, nullptr);
        ...
        for (int32_t j = 0; j < imDrawData->CmdListsCount; j++) {
            ...
            for (int32_t k = 0; k < cmd_list->CmdBuffer.Size; k++) {
                ...
                // Преобразуем координаты из координат UI viewport'а в координаты окна
                VkRect2D scissorRect;
                scissorRect.offset.x = viewport.x + std::max((int32_t)(pcmd->ClipRect.x), 0);
                scissorRect.offset.y = viewport.y + std::max((int32_t)(pcmd->ClipRect.y), 0);
                scissorRect.extent.width = (uint32_t)(pcmd->ClipRect.z - pcmd->ClipRect.x);
                scissorRect.extent.height = (uint32_t)(pcmd->ClipRect.w - pcmd->ClipRect.y);
